...
const addFooter = (doc: jsPDF) => {
  ...
  doc.setFontSize(10);
  doc.text(authorized, 10, footerY-4);

  // Adjust Y positions for the remaining footer elements
  doc.setFontSize(8);
  doc.text(footertitle, 10, footerY + 10);
  doc.text(footeremail, 80, footerY + 10);
  doc.text(footertele, 160, footerY + 10);
  doc.text(footerAddress, 10, footerY + 15);
  doc.text(footerMail, 80, footerY + 15);
  doc.text(footerTel, 160, footerY + 15);
...
export const generateCombinedPdf = (data: DataEntry[], profileData: ProfileData) => {
  ...
  const addTableWithPagination = (title: string, rows: [string, string][]) => {
    ...
    if (currentYPosition + tableHeight <= footerY) {
      ...
      currentYPosition = doc.lastAutoTable.finalY + tableSpacing;
    } else {
